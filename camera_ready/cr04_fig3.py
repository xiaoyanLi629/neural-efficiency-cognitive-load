#!/usr/bin/env python3
# Camera-ready copy of regenerate_paper_figures.py: Figs. 1 and 3 only, on smaller canvases so fonts stay legible at column width;
# Fig. 3 panel B relabelled (it plots whole-run Q, not the mean over loads).
"""
...
OUT_DIR = PROJECT_ROOT / 'camera_ready' / 'figs'
...
def add_stats(ax, x1, x2, y, label='***'):
    ...
    ax.text(0.5, y * 1.02, f'{sig}\n' + ('p < 0.001' if p < 0.001 else f'p = {p:.3f}') + f'\nd = {mean_d:.2f}',
...
def make_fig1():
    ...
    ax_c.legend(loc='center left', fontsize=9, framealpha=0.9, edgecolor='none')
...
def make_fig3():
    ...
    fig = plt.figure(figsize=(10.5, 3.3), facecolor='white')
    gs = gridspec.GridSpec(1, 3, hspace=0.3, wspace=0.5, figure=fig)
    ...
    ax_c.text(0.5, y_top * 1.08, f'{sig}  t = {t:.2f}\np = {p:.3f}, d = {cohen_d:.2f}',
              ha='center', va='bottom', fontsize=10, color=TEXT)
